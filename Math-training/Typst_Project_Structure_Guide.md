...
1. Source material lives outside the repository in `Project-source/Math/` or
   arrives as Markdown generated by the sibling `PDF-to-Markdown` project.
2. You manually author or refine a top-level `Note-to-*.typ` file.
...
- Reuse the style decisions already demonstrated in `Note-to-EGMO.typ`.
...
2. Treat `Note-to-EGMO.typ` as the style baseline.
...
1. Start from the source PDF in `../../Project-source/Math/` or from Markdown
   generated by the sibling `PDF-to-Markdown` repo.
...
3. Identify the supporting material that should become a collected results or
   appendix section.
4. Draft the content in a new or existing `Note-to-*.typ` file.
5. Reuse `utils/` helpers and the `Note-to-EGMO.typ` structure.
...
Input:
- Source material from Project-source/Math/... or Markdown converted by the
  PDF-to-Markdown project.

Goal:
- Update or create a top-level Note-to-BOOK.typ file.
- Follow the document style used by Note-to-EGMO.typ.
